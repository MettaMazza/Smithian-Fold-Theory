...
- **Solar reconnection particle acceleration** — Fermi acceleration in contracting magnetic islands *is* the fold's doubling, so reconnection protons are forced onto a binary energy tower $E_k = 2^k E_{\text{floor}}$ above the magnetic‑energy floor — a discrete log‑2 ladder no continuum model can produce. NASA's Parker Solar Probe (2025) found protons at ~1000× the magnetic energy per particle — the source *"no existing model predicted"* — sitting on the $2^{10}$ rung. The forced, falsifiable signature is the doubling spacing of the spectrum ([`reconnection_energy.py`](reconnection_energy.py); finding: [arXiv:2410.16539](https://arxiv.org/abs/2410.16539))
